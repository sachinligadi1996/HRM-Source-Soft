...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_image['size'] > 0) {
        ...
        $new_image_path = $upload_dir . basename($new_image['name']);

        if (move_uploaded_file($new_image['tmp_name'], $new_image_path)) {
            // Update the database with the new image path
            $update_query = "UPDATE assign_task SET upload = '$new_image_path' WHERE id = '$task_id'";
            $update_result = mysqli_query($conn, $update_query);

            if ($update_result) {
                // Redirect back to the view page after updating
                header("Location: emp_task.?id=$task_id");
                exit();
            } else {
    ...
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <form action="emp_task?id=<?php echo $task_id; ?>" method="post" enctype="multipart/form-data" class="mt-3" id="updateForm" onsubmit="return validateForm()">
                <div class="mb-3">
